...
import { getSeminars, getStudentEnrollments } from '../lib/firebase';
import {
  Calendar,
  BookOpen,
  Users,
    ...
    const loadDashboardData = async () => {
      try {
        if (userRole === 'student') {
          ...
          const seminars = await getSeminars({ limit: 5 });
          ...
          setStats({
            ...
            totalCredits: enrollments.reduce((sum, e) => sum + (e.seminar?.credits || 0), 0)
          });
          
          setUpcomingSeminars(seminars.slice(0, 3));
          setRecentActivity([
          ...
          const seminars = await getSeminars({ createdBy: user.uid });
          
          setStats({
            mySeminars: seminars.length,
            totalStudents: seminars.reduce((sum, s) => sum + (s.currentEnrollment || 0), 0),
  ...
  const getRoleSpecificQuickActions = () => {
    const actions = {
      ...
      admin: [
        ...
        { name: 'Settings', href: '/admin/settings', icon: Settings, color: 'purple' },
        { name: 'Reports', href: '/admin/reports', icon: FileText, color: 'orange' }
      ]
    };
  ...
  const getStatsCards = () => {
    const cards = {
      ...
      admin: [
        ...
        { name: 'Active Seminars', value: 89, icon: BookOpen, color: 'green' },
        { name: 'System Health', value: '99.9%', icon: CheckCircle, color: 'yellow' },
        { name: 'Monthly Growth', value: '+12%', icon: TrendingUp, color: 'purple' }
      ]
    };
